(function(){
    'use strict';
    angular.module('admin').controller('assignTest', ['$scope', '$state', 'userService', '$location', '$stateParams', 'notification', function($scope, $state, userService, $location, $stateParams, notification) {
        ...
        $scope.submitStudentsList = function() {
            ...
            $scope.data.availableOptions.forEach(function(item){
                listToSend.push(JSON.parse(item.user));
            });
            var objToSend = {};
            ...
            objToSend.date = {};
            objToSend.date.start = now.getTime();
            objToSend.date.end = tomorrow;
